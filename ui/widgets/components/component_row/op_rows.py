...
مستخرج من widgets/shared/component_row/_op_rows_logic.py مع:
  ...
  [إصلاح] _determine_target_id: استبدال `or` بـ explicit `is not None` checks.
    القديم: `selected_row_id or self._init_machine_op_row_id or self._pinned_op_row_id`
    المشكلة: لو أي قيمة = 0 (صفر) يتم تخطيها بسبب سلوك `or` في Python.
    رغم أن SQLite IDs تبدأ من 1، النمط نفسه خطر ويجب تجنبه.
    الحل: فحص صريح بـ `is not None`.

  [إصلاح هيكلة] _fetch_op_rows و _calc_row_cost يستوردان من services
    بدل db/ مباشرة. المسار الصحيح: widget → service → repo (db/)
"""
...
class OpRowsMixin:
    ...
    def _fetch_op_rows(self, op_id: int) -> list:
        """
        [إصلاح هيكلة] يستورد من services بدل db/ مباشرة.
        المسار الصحيح: widget → service → repo (db/)
        """
        try:
            from services.costing.machine_op_rows_service import get_op_rows
            conn = self._get_conn()
            if conn is None:
                return []
            return get_op_rows(conn, op_id)
        except Exception:
            return []
    ...
    def _populate_op_row_combo(self, rows: list, target_id: "int | None"):
        ...
        with blocked_signals(self.cmb_op_row):
            ...
            for row in rows:
                ...
                label   = row["label"] or tr('op_row_fallback_label').format(id=row['id'])
                value_str = f"{row['value']:.4g}"
                count_str = f"{row['count']:.4g}"
                cost_str  = f"{cost:.3f}"
                display = (
                    f"{label}  "
                    f"{tr('op_row_combo_fraction').format(value=value_str, count=count_str)}"
                    f"{tr('op_row_combo_approx_cost').format(cost=cost_str)}"
                )
    ...
    def _calc_row_cost(self, row_id: int) -> float:
        ...
        """
        try:
            from services.costing.machine_op_rows_service import get_op_row_cost
            conn = self._get_conn()
            if conn:
                return get_op_row_cost(conn, row_id)
        except Exception:
            pass
        return 0.0
    ...
    def _update_op_row_cost_label(self):
        ...
        self.lbl_op_row_cost.setText(tr('op_row_cost_suffix').format(cost=f"{cost:.4f}"))
