...
[v7]:
  - _on_company_event_safe محسّنة — تحدث _company_id تلقائياً.
  - DualConnMixin: مكسن إضافي لأي widget يحتاج acc_conn + erp_conn معاً.
  - get_active_company_id() موحدة مع company_utils.
"""
...
class SafeConnMixin:
    ...
    مكسن يوفر:
      ...
      _get_company_id()               → company_id النشط (static)
      _on_company_event_safe()        → منطق موحد للرد على company_data_changed
    """
