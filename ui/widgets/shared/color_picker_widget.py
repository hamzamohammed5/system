"""
ui/widgets/shared/color_picker_widget.py
=========================================
ColorPickerWidget — widget موحد لاختيار اللون.

يوحّد الكود المكرر في:
  - _GroupManagerPanel (group_manager.py)
  - _CategoryForm (category_form.py)
  - _AccountForm (tree/_account_form.py)
  - أي فورم فيه لون

الاستخدام:
    from ui.widgets.shared.color_picker_widget import ColorPickerWidget

    picker = ColorPickerWidget(default="#607d8b")
    form.add_row("اللون:", picker)
    color = picker.current_color()   # → "#607d8b"
    picker.set_color("#1565c0")
"""

from PyQt5.QtWidgets import QWidget, QHBoxLayout, QLabel, QPushButton, QColorDialog
from PyQt5.QtCore import pyqtSignal
from PyQt5.QtGui  import QColor

from ui.widgets.shared.panles_helper.make_btn import _make_btn


class ColorPickerWidget(QWidget):
    """
    Widget مدمج لاختيار اللون:
      [■ مربع اللون] [اختر لون]

    Signals:
        color_changed(str) — يُطلق عند تغيير اللون (hex)
    """

    color_changed = pyqtSignal(str)

    def __init__(self, default: str = "#607d8b",
                 btn_text: str = "اختر لون",
                 parent=None):
        super().__init__(parent)
        self._color = default
        self._build(btn_text)

    def _build(self, btn_text: str):
        self.setStyleSheet("background: transparent;")
        lay = QHBoxLayout(self)
        lay.setContentsMargins(0, 0, 0, 0)
        lay.setSpacing(6)

        self._lbl_preview = QLabel()
        self._lbl_preview.setFixedSize(28, 28)
        self._apply_preview()
        lay.addWidget(self._lbl_preview)

        btn = _make_btn(btn_text, "normal")
        btn.clicked.connect(self._pick)
        lay.addWidget(btn)
        lay.addStretch()

    def _apply_preview(self):
        self._lbl_preview.setStyleSheet(
            f"background:{self._color}; border-radius:4px;"
            "border:1px solid #ccc;"
        )

    def _pick(self):
        col = QColorDialog.getColor(QColor(self._color), self, "اختر لون")
        if col.isValid():
            self._color = col.name()
            self._apply_preview()
            self.color_changed.emit(self._color)

    def current_color(self) -> str:
        return self._color

    def set_color(self, color: str):
        self._color = color
        self._apply_preview()