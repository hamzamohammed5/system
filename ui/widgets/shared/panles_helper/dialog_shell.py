"""
ui/widgets/shared/panles_helper/dialog_shell.py
================================================
_DialogShell — هيكل نافذة حوار موحد بدون منطق.

يُستخدم داخلياً من:
  - message_box.py   (_MsgDialog)
  - confirm_dialog.py (_ConfirmDialog)
  - dialog_base.py   (BaseDialog)
...
الاستخدام:
    ...
    class MyDlg(_DialogShell):
        def __init__(self, parent=None):
            ...
            # أضف محتوى في self.body_layout
            self.body_layout.addWidget(QLabel("مرحباً"))
            # أضف أزرار في self.btn_layout
...
class _DialogShell(QDialog):
    """
    هيكل نافذة حوار موحد — بدون منطق.

    المعاملات:
        title     : عنوان يظهر في الهيدر
        icon      : emoji الأيقونة
        subtitle  : نص ثانوي في الهيدر (اختياري)
        accent    : لون الهيدر (hex)
        min_width : عرض أدنى للنافذة
        min_height: ارتفاع أدنى (0 = تلقائي)
    """
    ...
    def _build(self, icon: str, title: str, subtitle: str):
        ...
        body = QWidget()
        body.setStyleSheet(
            f"background: {_C.get('bg_page', '#f9f9f9')};"
        )
        ...
        sep.setStyleSheet(
            f"background: {_C.get('border', '#e0e0e0')}; border: none;"
        )
        root.addWidget(sep)
        ...
        bar = QWidget()
        bar.setStyleSheet(
            f"background: {_C.get('bg_surface', 'white')};"
        )
