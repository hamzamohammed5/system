"""
ui/widgets/shared/panles_helper/crud_section.py
================================================
CrudSection — قاعدة موحدة لأقسام CRUD الكاملة.

يجمع:
  - ListPanel  (قايمة + بحث)
  - DetailPanel (تفاصيل + تعديل)
  - FormPanel  (فورم إضافة/تعديل)

يحل التكرار في كل أقسام التطبيق اللي فيها:
  - جدول + تفاصيل على اليمين
  - فورم إضافة في البانل اليسار أو الأسفل

الاستخدام:
    class RawSection(CrudSection):
        LIST_TITLE   = "الخامات"
        DETAIL_TITLE = "تفاصيل الخامة"
        ADD_TEXT     = "➕ إضافة خامة"

        def _create_list(self):
            return RawListPanel(self.conn)

        def _create_detail(self):
            return RawDetailPanel(self.conn)

        def _create_form(self):
            return RawForm(self.conn)

    section = RawSection(conn)
...
class CrudSection(QWidget):
    ...
    Override:
      _create_list()   → لوحة القائمة (BaseListPanel أو أي QWidget)
      _create_detail() → لوحة التفاصيل (BaseDetailPanel أو أي QWidget)
      _create_form()   → فورم الإضافة/التعديل (اختياري — None لو مدمج في detail)
      _connect_signals() → ربط الـ signals بين الـ panels
    """
    ...
    def _create_form(self) -> QWidget | None:
        """يرجع None لو الفورم مدمج في الـ detail."""
        return None

    def _connect_signals(self):
        """Override لربط الـ signals بين الـ panels."""
        pass

    def _on_item_selected(self, item_id: int):
        """يُستدعى عند تحديد عنصر في القائمة."""
    ...
    def _build(self):
        ...
        # بناء الجانب الأيسر (قائمة + فورم اختياري)
    ...
    def _build_left(self) -> QWidget:
        """يبني الجانب الأيسر: قائمة + فورم (اختياري)."""
        ...
        # FORM_POSITION == "left" → الفورم بديل للقائمة (toggle)
        self._form = form
        return self._list
    ...
    def select_item(self, item_id: int):
        """يحدد عنصر في القائمة ويعرض تفاصيله."""
