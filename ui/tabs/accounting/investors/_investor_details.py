...
[إصلاح v6]:
  - fetch_investor_entry_id() من repo بدل SQL مباشر داخل _delete_movement().
"""
...
class _InvestorDetails(DualConnMixin, QWidget):
    ...
    def _delete_movement(self):
        ...
        try:
            erp = self._get_erp_conn()
            acc = self._get_safe_conn()
            # [إصلاح v6] fetch_investor_entry_id بدل SQL مباشر
            entry_id = fetch_investor_entry_id(erp, link_id)
            if entry_id is not None:
                try:
                    delete_entry(acc, entry_id)
