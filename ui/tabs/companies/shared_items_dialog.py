...
الفرق عن SharedItemsManagerDialog:
  ...
  - أسرع وأبسط
  - تظهر من داخل جداول الخامات / الماكينات / العمالة
"""
...
class SharedItemsDialog(QDialog):
    ...
    بعد الحفظ: bus.data_changed يُطلق → كل الجداول في كل الشركات تتحدث.
    """
    ...
    def __init__(self, central_conn, shared_id: int, parent=None):
        ...
        self.setStyleSheet(f"QDialog {{ background: #f5f7fa; }}")
        self._build()
        self._load()
    ...
    def _build_data_fields(self, shared_type: str, data: dict):
        ...
        if shared_type == "raw":
            ...
            lbl1 = QLabel("السعر الكلي (جنيه):")
            lbl1.setStyleSheet(lbl_ss)
            lbl2 = QLabel("الكمية الكلية (0 = بدون):")
            lbl2.setStyleSheet(lbl_ss)
            ...
            lbl1 = QLabel("معدل / ساعة (جنيه):")
            lbl1.setStyleSheet(lbl_ss)
            lbl2 = QLabel("معدل / وحدة (جنيه):")
            lbl2.setStyleSheet(lbl_ss)
    ...
    def _update_raw_preview(self):
        """تحديث معاينة سعر الوحدة للخامة."""
        try:
        ...
        for company in all_companies:
            ...
            cb.setStyleSheet(f"""
                QCheckBox {{
                    color: {color};
                    font-size: 12px;
                    font-weight: {'bold' if company['id'] in linked_ids else 'normal'};
                    padding: 4px 8px;
                    border-radius: 4px;
                }}
    ...
    def _save(self):
        ...
        try:
            # 1. تحديث بيانات العنصر في companies.db (يتعكس فوراً)
            update_shared_item(self._central, self._shared_id, name, data)

            # 2. تحديث ربط الشركات
            set_linked_companies(
                self._central, self._shared_id, selected_company_ids
            )
        ...
        # 3. إطلاق bus.data_changed → كل الجداول المفتوحة في كل الشركات تتحدث
        try:
