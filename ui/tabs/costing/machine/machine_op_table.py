...
class _MachineOpTable(SharedItemsListPanel):
    ...
    def _setup_column_widths(self, table):
        ...
        # [توحيد الجداول] حذف setAlternatingRowColors(True) اليدوي —
        # الخاصية دي متطبقة تلقائيًا مركزيًا جوه _build_table() في
        # tables.py، تكرارها هنا زيادة لا داعي لها وغير متسقة مع
        # باقي الجداول الوارثة من SharedItemsListPanel.
    ...
    def _fill_table_row(self, r: int, item: dict):
        ...
        self.table.setItem(r, 3, make_item(item.get("machine_name", tr("dash"))))
        self.table.setItem(r, 4, make_item(mode_ar))
        self.table.setItem(r, 5, make_item(f"{item.get('value', 0):.4f}"))
        self.table.setItem(r, 6, make_item(f"{cost:.2f} {tr('currency_abbr')}"))
