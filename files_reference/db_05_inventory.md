...
- حساب `avg_cost` تلقائي في `record_inventory_move()` — لا تحسبه يدوياً.
- للتعديل لكمية أقل في `adjust`: استخدم `move_type="out"` بدلاً من كمية سالبة.
- `inventory.db` منفصل عن `accounting.db` — الربط يتم عبر `ref_entry_id` بدون FK.