# دليل الكود — Models & Services (3): Services التكلفة
...
> **ملاحظة:** يخزّن نتيجة `PRAGMA table_info(bom)` في cache داخلي — استدعِ `invalidate_columns_cache()` بعد أي migration يضيف أعمدة لجدول bom، أو أنشئ instance جديد من الـ service.