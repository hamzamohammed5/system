...
- `form_mixins.py` (`EditModeMixin`): يُستخدم من `BaseCrudForm` (`ui_widgets_base.md`) — هو المستهلك الرئيسي
- `form_mixins.py` (`FormValidationMixin`): يُستخدم من `BaseCrudForm` وأي form يحتاج تحقق مباشر
- `shared_ops.py` (`SharedOpsMixin`): يُستخدم من كل جداول التكلفة التي تعرض عناصر مشتركة (`raw_table_panel.py`, `labor_op_table.py`, `machine_table.py` في `ui/tabs/costing/` — مؤكَّد من `ui_tabs_costing.md` الذي يذكر `to_dict` و`SHARED_COLOR` و`SHARED_BG`)
