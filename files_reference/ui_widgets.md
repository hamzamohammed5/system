...
- `_on_theme_changed(theme_name)` → يُعيد تطبيق styles على الـ widget والـ empty state والـ pagination bar والـ status bar
- `_on_language_changed(lang_code)` → يُحدّث placeholder البحث + نص empty state (عبر `_tr_safe(EMPTY_TITLE)`) + نصوص أزرار pagination
...
# يُحدّث نص _empty.set_title() بـ _tr_safe(EMPTY_TITLE)
```
...
ProgressBar(label="", color=None, height=8, show_pct=True, compact=False)
  ...
  .value() -> float
  .reset()
  # resizeEvent: يُعيد حساب عرض الـ fill تلقائياً — التحقق من total_w>0 مقصود ومهم
...
class MyPanel(QWidget, BusConnectedMixin):
    ...
    def _on_theme_changed(self, theme_name: str):
        # إعادة تطبيق الألوان من _C المحدث
...
# _get_conn(): بدون SELECT 1 overhead — يثق في الـ cache مباشرة
  ...
  ._refresh_cost_label()
  # يُحدّث lbl_variant_cost من الـ catalog الجديد
...
class MyListPanel(BaseListPanel):
    ...
    def _on_theme_changed(self, theme_name: str):
        ...
        # إضافة تحديثات مخصصة للثيم هنا

    def _on_language_changed(self, lang_code: str):
        super()._on_language_changed(lang_code)
        # إضافة تحديثات مخصصة للغة هنا
```
...
class MyForm(BaseCrudForm):
    FORM_TITLE = tr("category_data")

    def _build_fields(self, group: FormGroup):
        self.inp_name = RequiredLineEdit(tr("name"))
        group.add_row(f"{tr('name')} :", self.inp_name)
