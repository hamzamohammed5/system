...
# ══════════════════════════════════════════════════════════
# Bootstrapping — فتح اتصال designs.db وتهيئة الـ schema
# ══════════════════════════════════════════════════════════
# [إضافة] تغليف get_designs_connection + create_designs_tables هنا
# عشان tabs/design_section.py ميستدعيش db.designs.design_schema
# مباشرة (كسر هيكلي: tabs → repos/db بتجاوز services).
#
# قاعدة الـ Layering تبقى محفوظة:
#   tabs/  →  services/design.get_designs_conn_and_init()  →  db.designs.design_schema
#
# هذه الدالة bootstrapping بحت (فتح اتصال + تهيئة جداول)،
# وليست عملية بيانات — لذلك مكانها الطبيعي هنا في نقطة الدخول
# الموحّدة لـ services/design بدل تكرارها داخل كل service مستقل.

def get_designs_conn_and_init():
    """
    يفتح اتصال designs.db وينشئ/يهيّئ جداوله إن لم تكن موجودة، ثم يُرجع الاتصال.

    يُستخدم من tabs/design_section.py بدلاً من استدعاء
    db.designs.design_schema مباشرة، حفاظاً على القاعدة المعمارية:
