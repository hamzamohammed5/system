...
الجديد:
  ...
  2. عمود dpi — لحفظ دقة الكانفاس المختارة عند الإنشاء
"""
...
def run_migrations_v5(conn):
    ...
    # ══ 2. عمود dpi في design_sizes (للجداول الموجودة) ══
    if not _column_exists(conn, "design_sizes", "dpi"):
        conn.execute(
            "ALTER TABLE design_sizes ADD COLUMN dpi INTEGER NOT NULL DEFAULT 300"
        )
        conn.commit()